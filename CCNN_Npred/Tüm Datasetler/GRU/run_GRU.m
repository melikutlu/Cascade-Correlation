% GRU baseline for the CCNN datasets.
...
switch lower(config.training.loss_mode)
    case 'one-step'
        ...
        layers = buildRecurrentLayers(size(XTrain, 1), config, 'gru');
    ...
    case 'simulation'
        [net, trainInfo] = trainRecurrentSimulation(Utr, Ytr, Uva, Yva, config, 'gru');
    otherwise
...
save(fullfile(runFolder, 'GRU_Model.mat'), 'net', 'trainInfo', 'config', 'norm_stats', 'rmse_tr', 'rmse_va', 'fit_tr', 'fit_va');
writeBaselineLog(fullfile(runFolder, 'GRU_Log.txt'), 'GRU', config, rmse_tr, rmse_va, fit_tr, fit_va);
saveTrainingLossFigure(runFolder, 'GRU', trainInfo);
saveBaselineFigures(runFolder, 'GRU', Ytr_eval, Yhat_tr, Yva_eval, Yhat_va, fit_tr, fit_va);
