function Y = forwardModelTrajectory(X0, U, W_hidden, g, w_o, config)
    ...
    % Diferansiyel derecesini ve hidden katman sayisini al
    nHidden = numel(W_hidden);
    diffOrder = getDiffOrder(config);

    % Hidden katman yoksa model tamamen lineer kalir.
    % Bu durumda output agirlik egitimi/prediction sirasinda
    % aktivasyon veya diferansiyel donusume hic girilmez.
    if nHidden == 0
        for t = 1:N
            uvals = zeros(M, nu);
            for j = 1:nu
