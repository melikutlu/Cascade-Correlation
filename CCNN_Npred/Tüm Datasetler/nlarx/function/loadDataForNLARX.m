function [Utr, Ytr, Uva, Yva] = loadDataForNLARX(config)
    ...
    % Ensure config.data.source is set
    if ~isfield(config, 'data') || ~isfield(config.data, 'source')
        error('config.data.source must be specified: ''twotankdata'', ''dryer2'', or ''mrdamper''');
    end
    ...
    switch lower(config.data.source)
        case 'twotankdata'
            ...
            if isfield(config.data.twotank, 'filter_cutoff') && config.data.twotank.filter_cutoff > 0
                ...
                uf = zeros(size(u)); yf = zeros(size(y)); 
                uf(1)=u(1); yf(1)=y(1);
                for k=2:length(u)
            ...
            Utr = u(1:Ntr); Ytr = y(1:Ntr); 
            Uva = u(Ntr+1:end); Yva = y(Ntr+1:end);
