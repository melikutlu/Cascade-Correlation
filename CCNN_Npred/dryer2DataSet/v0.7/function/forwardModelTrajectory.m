function Y = forwardModelTrajectory(X0, U, W_hidden, g, w_o, config)
    ...
    act_order = getHiddenOrders(config, numel(W_hidden));
    z_state = cell(numel(W_hidden),1);
    for h=1:numel(W_hidden)
        z_state{h} = struct('prev1', dlarray(zeros(M,1)), 'prev2', dlarray(zeros(M,1)));
    end
    ...
    for t=1:N
        ...
        for h=1:numel(W_hidden)
            z = x * W_hidden{h};
            [a, z_state{h}] = applyHiddenActivation(z, z_state{h}, g, config, act_order(h));
            x = [x, a];
        end
        y = x * w_o;
        Y(:,t) = y;
