...
config.norm_method = 'ZScore'; % options: 'ZScore', 'minmax', 'None'
...
% model / training
config.model.activation = 'tanh'; % options: 'tanh', 'sigmoid', 'relu', 'leakyrelu', 'linear'
config.model.max_hidden_units = 15;
config.model.target_mse = 5e-5;
config.model.min_mse_improvement = 1e-6; % early stop threshold

% Quickprop typically settles within 100-300 epochs; plateau guard stops early.
...
config.model.quickprop.mu = 1.75;        % max growth multiplier for Quickprop
config.model.quickprop.epsilon = 1e-8;   % denominator guard for Quickprop
...
Yhat_tr = denormalizeSeries(Yhat_tr(2:end), norm_stats, 'y');
Yhat_va = denormalizeSeries(Yhat_va(2:end), norm_stats, 'y');
...
function [w_h, best_metric, info] = trainCandidateUnit_Corr(X0,U,T,W_hidden,w_o,g,config)
    % Train a candidate unit to maximize correlation^2 with the N-step residual via Quickprop.
    ...
    for ep=1:maxEpochs
        ...
        for b=1:numel(batches)
            ...
            [~, ~, gradDl] = dlfeval(@loss_candidate_corr, w_h_dl, Xb, Ub, Tb, W_hidden, w_o_d, g, config);
            grad = extractdata(gradDl);
            [w_h, qpState] = quickpropUpdate(w_h, grad, qpState, config.model.eta_candidate, config.model.quickprop.mu, config.model.quickprop.epsilon);
        end

        metricVal = gather(extractdata(candidateCorrelationMetric(w_h, X0, U, T, W_hidden, w_o, g, config)));
...
function [w_o,mse,info] = trainOutputLayer_Trajectory(X0,U,T,w_init,W_hidden,g,config)
    w_o = w_init;
    X0_d = dlarray(X0); U_d = dlarray(U); T_d = dlarray(T);
    ...
    for ep=1:maxEpochs
        ...
        for b=1:numel(batches)
            ...
            Xb = X0_d(idx,:); Ub = U_d(idx,:); Tb = T_d(idx,:);
            w_dl = dlarray(w_o);
            [Lbatch,gradDl] = dlfeval(@loss_output_traj, w_dl, Xb, Ub, Tb, W_hidden, g, config);
            grad = extractdata(gradDl);
            [w_o, qpState] = quickpropUpdate(w_o, grad, qpState, config.model.eta_output, config.model.quickprop.mu, config.model.quickprop.epsilon);
            batchLoss = gather(extractdata(Lbatch));
    ...
    Y = forwardModelTrajectory(X0_d, U_d, W_hidden, g, w_o, config);
    Yvec = reshape(Y,1,[]);
    Tvec = reshape(T_d,1,[]);
...
function [Utr,Ytr,Uva,Yva,stats] = normalizeData_min(method,Utr,Ytr,Uva,Yva)
    ...
    switch method
        case 'zscore'
            stats.u.mu = mean(Utr);
            stats.u.sigma = std(Utr) + eps;
            stats.y.mu = mean(Ytr);
            stats.y.sigma = std(Ytr) + eps;

            Utr = (Utr - stats.u.mu) / stats.u.sigma;
            Uva = (Uva - stats.u.mu) / stats.u.sigma;
            Ytr = (Ytr - stats.y.mu) / stats.y.sigma;
            Yva = (Yva - stats.y.mu) / stats.y.sigma;

            % legacy fields for backward compatibility
            stats.u_mu = stats.u.mu; stats.u_std = stats.u.sigma;
            stats.y_mu = stats.y.mu; stats.y_std = stats.y.sigma;

        case {'minmax','min-max'}
            stats.u.min = min(Utr);
            stats.u.max = max(Utr);
            stats.u.range = max(stats.u.max - stats.u.min, eps);
            stats.y.min = min(Ytr);
            stats.y.max = max(Ytr);
            stats.y.range = max(stats.y.max - stats.y.min, eps);

            Utr = (Utr - stats.u.min) / stats.u.range;
            Uva = (Uva - stats.u.min) / stats.u.range;
            Ytr = (Ytr - stats.y.min) / stats.y.range;
            Yva = (Yva - stats.y.min) / stats.y.range;
...
function data = denormalizeSeries(data, stats, fieldName)
    if ~isfield(stats, 'method')
        error('Normalization stats must include a method field.');
    end

    switch lower(stats.method)
        case 'zscore'
            sigma = stats.(fieldName).sigma;
            mu = stats.(fieldName).mu;
            data = data * sigma + mu;
        case {'minmax','min-max'}
            rangeVal = stats.(fieldName).range;
            minVal = stats.(fieldName).min;
            data = data * rangeVal + minVal;
        case {'none','identity'}
            % no-op
        otherwise
            error('Unknown normalization method "%s".', stats.method);
    end
end
...
function g = resolveActivation(name)
    ...
    switch lower(name)
        ...
        case {'linear','identity','none'}
            g = @(x) x;
        otherwise
            error('Unsupported activation "%s".', name);
    end
end
...
function vec = sanitizeVector(vec, limit)
    if nargin < 2 || isempty(limit)
        limit = inf;
    end
    vec(~isfinite(vec)) = 0;
    if isfinite(limit)
        vec = max(min(vec, limit), -limit);
    end
end

function [w_new, state] = quickpropUpdate(w, grad, state, eta, mu, epsilon)
    if nargin < 6 || isempty(epsilon)
        epsilon = 1e-8;
    end

    grad = sanitizeVector(grad, 1e6);
    w = sanitizeVector(w, inf);
    if nargin < 3 || isempty(state)
        state.prev_grad = zeros(size(w));
        state.prev_step = zeros(size(w));
        state.iter = 0;
    end
    ...
    if state.iter == 1
        ...
        prev_grad = sanitizeVector(state.prev_grad, 1e6);
        prev_step = sanitizeVector(state.prev_step, 1e6);
        denom = prev_grad - grad;
        signDen = sign(denom);
        absDen = abs(denom);
        zeroMask = signDen == 0;
        signDen(zeroMask) = 1;
        denom = signDen .* max(absDen, epsilon);
        step = (grad ./ denom) .* prev_step;
        sign_change = (grad .* prev_grad) < 0;
        step(sign_change) = -eta * grad(sign_change);
    ...
    w_new = w + step;
    if any(~isfinite(step(:))) || any(~isfinite(w_new(:)))
        step = -eta * grad;
        step = sanitizeVector(step, 1e6);
        w_new = w + step;
    end
    state.prev_grad = grad;
    state.prev_step = sanitizeVector(step, 1e6);
end
