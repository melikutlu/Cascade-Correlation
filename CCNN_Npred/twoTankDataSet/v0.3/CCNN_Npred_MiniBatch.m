...
config.prediction.n_steps = 20; % user-selected N-step horizon for training trajectories
config.prediction.auto_full_horizon = false; % keep manual N to avoid full-length sequences
...
config.model.eta_candidate = 0.09;
...
config.model.plateau_extra_epochs = 10;  % run this many extra epochs after plateau trigger

config.training = struct();
config.training.batch_size = 32;         % mini-batch size for trajectory subsets
config.training.shuffle_each_epoch = true;
config.training.quickprop = struct('lr', 0.05, 'max_growth', 1.75, 'eps', 1e-8);
...
[w_o, current_mse, outputTrainInfo] = trainOutputLayer_Trajectory(X0_tr, Utr_seq, Ttr_seq, w_o, W_hidden, g, config, 'quickprop');
...
function [w_h, best_metric, info] = trainCandidateUnit_Corr(X0,U,T,W_hidden,w_o,g,config)
    ...
    qpCfg = getQuickpropConfig(config);
    best_metric = -Inf; best_w = extractdata(w_h);
    ...
    extraPatience = getPlateauExtraEpochs(config);
    plateauDetectedEpoch = NaN;

    prevGrad = zeros(size(extractdata(w_h)));
    prevDelta = zeros(size(prevGrad));
    wNumeric = gather(extractdata(w_h));
    epsQuick = qpCfg.eps;

    for ep=1:maxEpochs
        w_h = dlarray(wNumeric);
        [~, metric, grad] = dlfeval(@loss_candidate_corr, w_h, X0_d, U_d, T_d, W_hidden, w_o_d, g, config);
        gradVal = gather(extractdata(grad));
        delta = zeros(size(gradVal));
        sameSign = gradVal .* prevGrad > 0;
        if any(sameSign)
            denom = prevGrad(sameSign) - gradVal(sameSign);
            denomAbs = abs(denom);
            denomAbs(denomAbs < epsQuick) = epsQuick;
            denom = sign(denom) .* denomAbs;
            deltaQuick = (gradVal(sameSign) ./ denom) .* prevDelta(sameSign);
            cap = qpCfg.max_growth * prevDelta(sameSign);
            posMask = prevDelta(sameSign) > 0;
            deltaQuick(posMask) = min(deltaQuick(posMask), cap(posMask));
            deltaQuick(~posMask) = max(deltaQuick(~posMask), cap(~posMask));
            delta(sameSign) = deltaQuick;
        end
        delta(~sameSign) = -qpCfg.lr * gradVal(~sameSign);
        wNumeric = wNumeric + delta;
        prevDelta = delta;
        prevGrad = gradVal;

        epochMetric = gather(extractdata(metric));
        metric_hist(ep) = epochMetric;
        if epochMetric - best_metric > minDelta
            best_metric = epochMetric;
            best_w = extractdata(w_h);
            epochsSinceBest = 0;
            plateauDetectedEpoch = NaN;
        else
            epochsSinceBest = epochsSinceBest + 1;
            if isnan(plateauDetectedEpoch) && epochsSinceBest >= patience
                plateauDetectedEpoch = ep;
            end
            if epochsSinceBest >= patience + extraPatience
                if isnan(plateauDetectedEpoch)
                    plateauDetectedEpoch = ep;
                end
                plateauEpoch = plateauDetectedEpoch;
                break;
            end
        end
    end
...
function [w_o,mse,info] = trainOutputLayer_Trajectory(X0,U,T,w_o,W_hidden,g,config, optimizerMode)
    if nargin < 8 || isempty(optimizerMode)
        optimizerMode = 'adam';
    end
    ...
    switch lower(optimizerMode)
        case 'quickprop'
            qpCfg = getQuickpropConfig(config);
            bestLoss = inf;
            epochsSinceBest = 0;
            plateauDetectedEpoch = NaN;
            prevGrad = zeros(size(extractdata(w_o)));
            prevDelta = zeros(size(prevGrad));
            wNumeric = gather(extractdata(w_o));
            epsQuick = qpCfg.eps;

            for ep=1:maxEpochs
                w_o = dlarray(wNumeric);
        ...
        otherwise
            ...
            batchSize = getBatchSize(config, numSamples);

            for ep=1:maxEpochs
                if isfield(config,'training') && isfield(config.training,'shuffle_each_epoch') && config.training.shuffle_each_epoch
                ...
                epochLossSum = 0;
                epochCount = 0;
                for startIdx = 1:batchSize:numSamples
                    batchIdx = perm(startIdx:min(startIdx+batchSize-1, numSamples));
                    Xb = X0(batchIdx,:);
                    Ub = U(batchIdx,:);
                    Tb = T(batchIdx,:);
                    it = it+1;
                    ...
                    lossVal = gather(extractdata(L));
                    epochLossSum = epochLossSum + lossVal * numel(batchIdx);
                    epochCount = epochCount + numel(batchIdx);
                end
                epochLoss = epochLossSum / max(epochCount,1);
...
function batchSize = getBatchSize(config, numSamples)
    if ~isfield(config,'training') || ~isfield(config.training,'batch_size') || config.training.batch_size <= 0
        batchSize = numSamples;
    else
        batchSize = min(config.training.batch_size, numSamples);
    end
end
...
function extra = getPlateauExtraEpochs(config)
    extra = 0;
    if isfield(config,'model') && isfield(config.model,'plateau_extra_epochs')
        extra = max(0, config.model.plateau_extra_epochs);
    end
end

function qpCfg = getQuickpropConfig(config)
    qpCfg = struct('lr', 0.05, 'max_growth', 1.75, 'eps', 1e-8);
    if isfield(config,'training') && isfield(config.training,'quickprop')
        qp = config.training.quickprop;
        if isfield(qp,'lr'); qpCfg.lr = qp.lr; end
        if isfield(qp,'max_growth'); qpCfg.max_growth = qp.max_growth; end
        if isfield(qp,'eps'); qpCfg.eps = qp.eps; end
    end
end
