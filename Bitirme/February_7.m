...
% --- Recursive N-step ---
config.prediction.n_steps = 10;   % N
...
config.model.eta_candidate        = 0.01;
...
fprintf('Train: %d samples | Val: %d samples\n', size(U_train_raw,1), size(U_train_raw,1));
...
% İlk adımı çıkar (çünkü o gerçek değer)
Yhat_train_recursive = Yhat_train_recursive(2:end);  % k=2'den itibaren tahminler

% Karşılaştırma için train verisinin de ilk adımını çıkar
% Y_train_raw yerine Y_train_compare kullan!
Y_train_compare = Y_train_raw(2:end);  % k=2'den itibaren gerçek değerler

% Denormalize et (sadece tahminleri)
Yhat_train_real = denormalizeData_min(Yhat_train_recursive, config.norm_method, norm_stats.y);

% Fit hesapla (ilk adım hariç) - Y_train_compare kullan!
fit_train_full = fitPercent(Y_train_compare, Yhat_train_real);
fprintf('Full Series Training Fit (k=2:%d): %.2f%%\n', length(Y_train_raw), fit_train_full);

% ===== PLOT: TRAIN (Full Series) =====
figure('Color','w','Name','TRAIN - Full Series Recursive Prediction');
time_train = 2:length(Y_train_raw);  % k=2'den başla
plot(time_train, Y_train_compare, 'k', 'LineWidth', 1.5); hold on;
plot(time_train, Yhat_train_real, 'b--', 'LineWidth', 1.2);
grid on;
xlabel('Time step (k)');
ylabel('y (real units)');
title(sprintf('TRAIN - Recursive Prediction (k≥2) | Hidden=%d | Fit=%.2f%%', ...
...
% İlk adımı çıkar
Yhat_val_recursive = Yhat_val_recursive(2:end);  % k=2'den itibaren tahminler

% Karşılaştırma için val verisinin de ilk adımını çıkar
Y_val_compare = Y_val_raw(2:end);  % k=2'den itibaren gerçek değerler

% Denormalize et
Yhat_val_real = denormalizeData_min(Yhat_val_recursive, config.norm_method, norm_stats.y);

% Fit hesapla (ilk adım hariç) - Y_val_compare kullan!
fit_val_full = fitPercent(Y_val_compare, Yhat_val_real);
fprintf('Full Series Validation Fit (k=2:%d): %.2f%%\n', length(Y_val_raw), fit_val_full);

% ===== PLOT: VAL (Full Series) =====
figure('Color','w','Name','VAL - Full Series Recursive Prediction');
time_val = 2:length(Y_val_raw);  % k=2'den başla
plot(time_val, Y_val_compare, 'k', 'LineWidth', 1.5); hold on;
plot(time_val, Yhat_val_real, 'r--', 'LineWidth', 1.2);
grid on;
xlabel('Time step (k)');
ylabel('y (real units)');
title(sprintf('VAL - Recursive Prediction (k≥2) | Hidden=%d | Fit=%.2f%%', ...
...
function [w_h, v_h, current_mse] = trainCandidateUnit_Trajectory( ...
    ...
    % MSE'yi hesapla (bilgi için)
    W_hidden_temp = W_hidden;
    W_hidden_temp{end+1} = w_h;
    w_o_temp = [w_o_current; 0.1];
...
function [loss, grad_w] = loss_and_grad_candidate_traj_fixed(...
    ...
    for t = 1:Npred
        ...
        % DÜZELTME: Çıkış ağırlığını 0.1 yapıyoruz ki türev akabilsin
        w_o_temp = [w_o_current; 0.1];
