...
function [L,grad] = loss_candidate_traj(w_h,X0,U,T,W_hidden,w_o,g,config)
    % candidate unit produces an additional hidden activation; build merged hidden list
    Wtmp = [W_hidden, {w_h}];
    wtmp = [w_o; 0.1];
    Y = forwardCCNN_recursiveTrajectory(X0, U, Wtmp, g, wtmp, config);
    L = mean((T - Y).^2, 'all');
...
function Y = forwardCCNN_recursiveTrajectory(X0,U,W_hidden,g,w,config)
    ...
    for t = 1:N
        % build u regressor columns for all samples
        uvals = zeros(M, nu, 'like', X0);
        for j=1:nu
            L = ulags(j);
            if L==0
                uvals(:,j) = U(:,t);
        ...
        % build y regressor columns for all samples
        yvals = zeros(M, ny, 'like', X0);
        for j=1:ny
            L = ylags(j);
            if t - L >= 1
                yvals(:,j) = Y(:, t - L);
...
function [X0, Useq, Tseq] = createTrajectoryDataset(U, Y, config, N)
    ...
    % compute maximum past lag required (exclude ulags==0)
    maxLag = 0;
    if ~isempty(ulags(ulags>0))
        maxLag = max(maxLag, max(ulags(ulags>0)));
    end
    if ~isempty(ylags)
    ...
    if Ns < 1
        error('Not enough data for requested prediction horizon and regressors.');
    end
    ...
    Tseq = zeros(Ns, N);
    for idx = 1:Ns
        i = idx + maxLag; % align so first prediction time is k = i+1 and past lags exist
        row = zeros(1,nu+ny);
        for j=1:nu
            L = ulags(j);
            if L==0
                row(j) = U(i); % store previous u for compatibility (not necessarily used)
            else
                row(j) = U(i+1 - L);
            end
        end
