...
% activation (use identity when you don't want tanh/bentanh)
% activation is chosen via config.model.activation (string or function handle)
% do not define `g` here so user-selected algorithm can be passed through config
...
while numel(W_hidden) < config.model.max_hidden_units
    ...
    if ~isnan(candInfo.plateau_epoch)
        ...
        fprintf('Selected candidate used %d/%d epochs (no plateau).\n', ...
            candInfo.epochs_run, candInfo.epochs_run, config.model.max_epochs_candidate);
    end
