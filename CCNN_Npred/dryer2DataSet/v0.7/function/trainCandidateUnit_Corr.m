function [w_h, best_metric, info] = trainCandidateUnit_Corr(X0,U,T,W_hidden,w_o,g,config,candOrder)
    % Train a candidate unit to MAXIMIZE correlation^2 with the N-step residual using mini-batches.
    if nargin < 8 || isempty(candOrder)
        candOrder = 1;
    end
    ...
    for ep=1:maxEpochs
        ...
        for b=1:numel(batches)
            ...
            Tb = T_d(idx,:);
            it = it + 1;
            [loss, ~, grad] = dlfeval(@loss_candidate_corr, w_h, Xb, Ub, Tb, W_hidden, w_o_d, g, config, candOrder);
            [w_h, avgG, avgGSq] = adamupdate(w_h, grad, avgG, avgGSq, it, config.model.eta_candidate);
        end

        metricVal = evaluateCandidateMetric(w_h, X0_d, U_d, T_d, W_hidden, w_o_d, g, config, candOrder);
    ...
    info = struct('epochs_run', epochs_run, 'plateau_epoch', plateauEpoch, 'metric_history', metric_hist, 'selected_order', best_order, 'cand_order', candOrder);
end
